...
class CourseHandler(utils.BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            #progress percentages for powerful ideas to work out the fill level for the badges on the dashboard
            pi_proportion = ['Mixing', 'Comparing', 'Scaling', 'Sharing', 'Trading off']
            pi_uncertainty = ['Making decisions', 'Playing', 'Simulating']
            pi_representation = ['Interpreting data', 'Interpreting charts', 'Comparing groups']
            pi_pattern = ['Appreciating', 'Tiling', 'Constructing']
            pi_measurement = ['Reading scales', 'Converting', 'Estimating', 'Quantifying']
            pi_eocq = ['End of course questionnaire']
            progress_tracker = progress.UnitLessonCompletionTracker(course)
            pi_proportion_percent = progress_tracker.get_powerful_idea_percent_complete(student, pi_proportion)
            pi_uncertainty_percent = progress_tracker.get_powerful_idea_percent_complete(student, pi_uncertainty)
            pi_representation_percent = progress_tracker.get_powerful_idea_percent_complete(student, pi_representation)
            pi_pattern_percent = progress_tracker.get_powerful_idea_percent_complete(student, pi_pattern)
            pi_measurement_percent = progress_tracker.get_powerful_idea_percent_complete(student, pi_measurement)
            pi_eocq_percent = progress_tracker.get_powerful_idea_percent_complete(student, pi_eocq)

            #work out percentage for whole course to see if it is complete or not. If so, this makes the end of course questionnaire link active.
            pi_all = ['Mixing', 'Comparing', 'Scaling', 'Sharing', 'Trading off', 'Making decisions', 'Playing', 'Simulating', 'Interpreting data', 'Interpreting charts', 'Comparing groups', 'Appreciating', 'Tiling', 'Constructing', 'Reading scales', 'Converting', 'Estimating', 'Quantifying']
            pi_all_percent = progress_tracker.get_powerful_idea_percent_complete(student, pi_all)
            pi_complete = False
            if pi_all_percent == 100:
                pi_complete = True

            #get the individual unit percentages and then work the fill level of progress for each unit box on the dashboard
            unit_percents = progress_tracker.get_unit_percent_complete_by_title(student)
            for key in unit_percents:
                unit_percents[key] = self.calculate_fill_level(unit_percents[key] * 100)

            #add the fill values, unit percentages and whether the course is complete or not to the template when rendering so we can build dashboard up for student
            self.template_value['proportion_percentage'] = self.calculate_fill_level(pi_proportion_percent)
            self.template_value['uncertainty_percentage'] = self.calculate_fill_level(pi_uncertainty_percent)
            self.template_value['representation_percentage'] = self.calculate_fill_level(pi_representation_percent)
            self.template_value['pattern_percentage'] = self.calculate_fill_level(pi_pattern_percent)
            self.template_value['measurement_percentage'] = self.calculate_fill_level(pi_measurement_percent)
            self.template_value['eocq_percentage'] = self.calculate_fill_level(pi_eocq_percent)
            self.template_value['unit_percentages'] = unit_percents
            self.template_value['course_complete'] = pi_complete
        finally:
    ...
    #method to determine fill level for boxes on the dashboard.
    def calculate_fill_level(self, percent):
        fill = 0
